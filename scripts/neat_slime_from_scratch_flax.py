...
import time # Added for potential throttling/timing

# --- RL Specific Imports ---
import gym
import slimevolleygym # Registers the environment

# --- Helper Functions (Mostly Unchanged, Minor Edits Possible) ---

def convert_params_to_json_serializable(params):
    # This function likely remains the same
...
def draw_graph(trained_model_state, activations_list, cfg):
    # This function should still work, assuming cfg has num_inputs/num_output
    # It visualizes the *policy network* structure.
    G = nx.DiGraph()
    ...
    # Adjust access if params are nested (common with TrainState)
    param_dict_maybe_nested = trained_model_state.params
    param_dict = param_dict_maybe_nested.get('params', param_dict_maybe_nested) # Handle potential nesting
    ...
    # --- Node and Edge Creation (Ellipsis handling logic) ---
    num_elements = cfg.network.num_inputs # Now observation space size
    ...
    for i, node in enumerate(start_nodes):
        if node == 'obs_ellipsis':
            ...
            y_pos = adj_idx - (total_vis_nodes - 1) / 2.0 # Ensure float division
            G.add_node(node, pos=(0, y_pos))

    prev_layer_node_names_for_edges = [n for n in start_nodes if 'ellipsis' not in n]
    layer_distance = 0.5
    edge_weights_dict = {}
    layer_items = list(param_dict.items()) # Use the potentially nested param_dict
    ...
    for layer_index, (layer, value) in enumerate(layer_items):
        ...
        for node_i_vis in range(logical_vis_count):
            ...
            y_pos = node_i_vis - (logical_vis_count - 1) / 2.0 # Float division
            ...
            if is_ellipsis:
                ...
                for prev_node_name in prev_layer_node_names_for_edges:
                     try:
                         ...
                         # Handle input node naming convention
                         elif parts[0] == 'obs' and parts[1] == 'node':
                              prev_node_actual_idx = int(parts[2]) - 1 # 'obs_node_1' -> index 0
                         else: continue
    ...
    layer_items_for_text = list(param_dict.items()) # Use the potentially nested dict
    count = 1
    for i, (layer, value) in enumerate(layer_items_for_text):
         ...
         if not is_output_layer: # Hidden Layers
            ...
            count += 1
             ...
             # No activation shown for output logits typically
    ...
    nx.draw_networkx_edge_labels(
        ...
        label_pos=0.3, bbox=dict(alpha=0) # Adjusted label position
    )

    plt.tight_layout()
    plt.axis('off')

# --- Activation Definitions (Unchanged) ---
...
# --- Model Definition (GenomeClassifier - Unchanged) ---
class GenomeClassifier(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = self.layer_definitions[-1](x) # Output layer (logits)
        return x

# --- Layer Creation and Parameter Copying (Mostly Unchanged) ---
def create_layers(rng, layer_sizes, num_output, current_activations=None):
    # This function remains the same conceptually
    layers = []
    activations = []
    ...
    for i, hidden_size in enumerate(layer_sizes):
        ...
        if current_activations is None:
            ...
            # activations.append(POSSIBLE_ACTIVATION_FNS[chosen_index])
            default_activation = nn.tanh
            activations.append(default_activation)
        else:
            activations.append(current_activations[i])
    layers.append(nn.Dense(features=num_output)) # Output layer for action logits
...
def copy_params_and_create_model(rng, layer_sizes, current_activations, num_output, num_inputs, source_params=None):
    # This function remains the same conceptually, copies weights between potentially different architectures
    ...
    # Ensure dummy input matches observation shape (add batch dim if needed, but init usually handles it)
    ...
    # --- Parameter Copying Logic (remains the same) ---
...
# --- NEAT Mutation Functions (Unchanged logic, just pass cfg correctly) ---
...
def select_action(apply_fn, params, obs, rng):
    """Selects an action based on policy logits."""
    logits = apply_fn({'params': params}, obs)
    # Sample action from the categorical distribution defined by logits
    action = jax.random.categorical(rng, logits)
    log_prob = jax.nn.log_softmax(logits)[action] # Log prob of the chosen action
...
def calculate_discounted_returns(rewards, gamma):
    ...
    # Convert to numpy array and potentially normalize (optional, can help stability)
    returns = jnp.array(returns)
    # returns = (returns - jnp.mean(returns)) / (jnp.std(returns) + 1e-8) # Optional normalization
    return returns

def calculate_policy_loss(params, apply_fn, observations, actions, log_probs_old, returns):
    """Calculates the REINFORCE policy gradient loss."""
    # Get log probs of actions taken under *current* params
    # Use vmap for efficiency if observations/actions are batched from multiple episodes
    # For simplicity here, assume inputs are from a single episode trajectory
    logits = vmap(apply_fn, in_axes=({'params': None}, 0))({'params': params}, observations)
    all_log_probs = jax.nn.log_softmax(logits)

    # Select the log_probs corresponding to the actions actually taken
    log_probs_taken = jnp.take_along_axis(all_log_probs, actions[:, None], axis=1).squeeze()

    # REINFORCE loss: - E[ G_t * log(pi(a_t | s_t)) ]
    # We approximate the expectation with our collected trajectory
    loss = -jnp.mean(returns * log_probs_taken)

    # Optional: Add entropy bonus to encourage exploration (common in PG methods)
    # entropy = -jnp.mean(jnp.sum(jax.nn.softmax(logits) * jax.nn.log_softmax(logits), axis=1))
    # loss -= cfg.training.entropy_coeff * entropy

    return loss # We want to minimize this negative expected return
...
def train_step_rl(state, observations, actions, log_probs_old, returns):
    ...
    # Define grad function for the policy loss
    grad_fn = jax.value_and_grad(calculate_policy_loss, argnums=0) # Differentiate w.r.t. params (arg 0)

    # Calculate loss and gradients
    loss, grads = grad_fn(state.params['params'], state.apply_fn, observations, actions, log_probs_old, returns)

    # Apply gradients
    # Ensure grads structure matches params structure (might need {'params': grads} )

    return state, loss

def run_episode(env, state, rng):
    """Runs a single episode using the current policy and collects experience."""
    observations, actions, rewards, log_probs = [], [], [], []
    obs = env.reset()
    # Ensure obs is float32 from the start
    ...
    while not terminated:
        rng, act_rng = jax.random.split(rng)
        # obs should already be the correct type now
        obs_input = obs
        ...
        # --- Step environment with the MultiBinary action ---
        next_obs, reward, terminated, info = env.step(action_multibinary)
        next_obs = jnp.asarray(next_obs, dtype=jnp.float32) # Ensure next_obs is also float32

        # --- Store data for training ---
        observations.append(obs_input)
        # Store the DISCRETE action that the policy chose and whose log_prob we have
    ...
    # Return the list of DISCRETE actions along with other trajectory data
    return observations, actions, rewards, log_probs, total_reward, rng

def evaluate_policy(env, state, rng, num_eval_episodes=10):
    """Evaluates the policy's average reward over multiple episodes."""
    total_rewards = []
...
# --- Main Function ---
@hydra.main(version_base=None, config_path="../task", config_name="config_slime") # Use a specific config
def main(cfg):
    ...
    # Consider using wrappers if needed (e.g., for frame stacking, although not typical for SlimeVolley)
    try:
        env = gym.make(cfg.env.name)
        print(f"Initialized environment: {cfg.env.name}")
    except Exception as e:
        print(f"Error creating environment '{cfg.env.name}': {e}")
        return

    # Get observation and action space sizes from the environment
    num_inputs = env.observation_space.shape[0]
    # Assuming Discrete(8) mapping for SlimeVolley's MultiBinary(3) space
    # If env.action_space is directly Discrete, use env.action_space.n
    if isinstance(env.action_space, gym.spaces.Discrete):
        num_output = env.action_space.n
         ...
         num_output = 8 # 2^3 = 8 possible actions
    else:
        raise ValueError(f"Unsupported action space type: {type(env.action_space)}")

    # Update config values dynamically if needed (optional)
    ...
    optimizer = optax.adam(learning_rate=cfg.training.lr) # Adam often works better for RL

    # --- Reward Tracking & Detailed Log ---
    ...
    detailed_log_data = []
    generation_avg_rewards = [] # Track average rewards
    ...
    for generation in range(cfg.training.generations):
        ...
        # --- Create TrainState ---
        # Ensure params are correctly structured before creating state
        ...
        for _ in tqdm(range(cfg.training.episodes_per_generation), desc="Episodes"):
            ...
            all_rews.append(rews) # Keep rewards per episode for return calculation
            all_log_probs.extend(log_ps) # Store log_probs from collection time
        ...
        if not all_obs:
            ...
            continue # Skip to next generation if no data

        observations_np = jnp.array(all_obs)
        actions_np = jnp.array(all_acts)
        log_probs_old_np = jnp.array(all_log_probs)

        # Calculate discounted returns for all episodes and concatenate
        all_returns = []
        ...
        # Check shapes
        # print(f"Shapes for update: Obs={observations_np.shape}, Acts={actions_np.shape}, Returns={returns_np.shape}")
        if not (observations_np.shape[0] == actions_np.shape[0] == returns_np.shape[0]):
             ...
             continue # Skip update if shapes don't match

        # --- Update Phase ---
        print("Updating policy...")
        start_update_time = time.time()
        # Perform multiple optimization steps per generation if desired (like PPO)
        # For basic REINFORCE, one update step on the collected batch is typical
        model_state, loss = train_step_rl(
            model_state, observations_np, actions_np, log_probs_old_np, returns_np
        )
        update_time = time.time() - start_update_time
        print(f"Update complete. Loss: {loss:.4f} (Time: {update_time:.2f}s)")

        # --- Store and Save Reward Summary ---
        ...
        try:
            ...
            gen_data = {
                ...
                "average_reward": float(avg_reward_gen), # Use avg reward from collection
                "structure": {
                ...
                "training_stats": {
                    "loss": float(loss), # Store loss from the update step
        ...
        # Use the updated parameters for mutation source
        current_params_for_mutation = model_state.params['params'] # Get the actual learned params

        next_model = model # Start with current topology def
        next_params = model_state.params # Start with current learned weights
        next_layer_sizes = current_layer_sizes
        next_activations = current_activations
        model_updated = False # Flag to track if topology changed
        ...
        # If no topological mutation occurred, next_params still holds the *learned* weights
        # from the previous generation. If a mutation *did* occur, next_params holds
        # the *copied/initialized* weights for the new structure.

        # Update state for the next generation
        model = next_model
        params = next_params # These are the parameters (potentially mutated) for the *next* generation
    ...
    try:
        ...
        rewards_from_csv = []
        # Read data from CSV, skipping header
                ...
                if row: # Ensure row is not empty
                    try:
                        ...
                        rewards_from_csv.append(float(row[1]))
                         ...
                         print(f"Warning: Skipping malformed row in CSV: {row} ({parse_err})")
        ...
        if generations_from_csv and rewards_from_csv:
         ...
         print(f"Error: Could not find reward summary file '{reward_summary_file}' for plotting.")
    except Exception as e:
        print(f"Error generating reward plot from CSV: {e}")

    # Close the environment
    env.close()

if __name__ == "__main__":
    # Assumes you have a config file named 'config_slimevolley.yaml'
    # in the same directory or specified via command line.
    main()